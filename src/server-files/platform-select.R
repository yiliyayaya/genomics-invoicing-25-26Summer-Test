create_platform_select <- function(options_list) {
  ...
  select_container <- div(class = "platform-btn-container",
      lapply(seq_along(options_list), function(i) {
        btn_id <- paste0("btn_platform_", gsub("[^a-zA-Z0-9]", "_", options_list[i]))
        onclick_code <- sprintf("Shiny.setInputValue('btn_platform_click', '%s', {priority: 'event'});", options_list[i])
